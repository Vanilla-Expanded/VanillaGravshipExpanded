...
{
    public class JobDriver_ExtinguishAstrofiresNearby : JobDriver
    {
        ...
        protected Astrofire TargetFire => (Astrofire)job.targetA.Thing;
        ...
        public override IEnumerable<Toil> MakeNewToils()
        {
            ...
            {
                Thing thing2 = GenClosest.ClosestThingReachable(pawn.Position, pawn.Map, ThingRequest.ForDef(VGEDefOf.VGE_Astrofire), PathEndMode.Touch, TraverseParms.For(pawn), 9999f, (Thing f) => !job.GetTargetQueue(TargetIndex.A).Contains(f) && pawn.CanReserve(f), null, 0, 13);
                if (thing2 != null)
                {
                    ...
                    {
                        Astrofire fire = (Astrofire)(Thing)f1;
                        Astrofire fire2 = (Astrofire)(Thing)f2;
        ...
        private bool StartBeatingFireIfAnyAt(IntVec3 cell, Toil nextToil)
        {
            ...
            {
                Astrofire fire = thingList[i] as Astrofire;
        ...
        public bool TryBeatFire(Astrofire targetFire)
        {
